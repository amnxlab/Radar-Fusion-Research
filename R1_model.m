% R2 FMCW Radar Model — 5.8 GHz (Guarded N=1 / Multi-Sweep Modes)
...
SWERLING_MEAN_RCS_DBSM = -17;       % Mean RCS in dBsm for UAV at 5.8 GHz
...
BASE_SEED   = 54321;                % used only if RNG_MODE == "fixed"
...
SNR_dB     = 0.11;        % dB (post-LNA SNR unless NOISE_SINGLE_SWEEP_ONLY=true)
...
%% ===================== 4) TX Visualizations (time, inst-freq, FFT, spectrograms) =====================
if PLOT_TX_RX
...
%% ===================== 5) RX Visualizations (time, inst-freq, FFT, spectrogram) =====================
if PLOT_TX_RX
...
%% ===================== 6) Reshape, Dechirp, (UP-ONLY selection), Doppler =====================
...
% UP-sweep detection from TX phase slope
...
beatM_detect = beatM_full;            % detector source
...
if TEST_SINGLE_SWEEP
    ...
    % Optional: report Doppler using slow-time — NEVER used by detector
...
%% ===================== 7) Multi-Chirp Inst-Freq Plot (for insight, plot-only) =====================
...
if USE_SWERLING_II && PLOT_SWERLING
    figure('Name', 'Swerling II RCS Fluctuation (5.8 GHz)', 'Color', 'w', 'Position', [100, 100, 1200, 800]);
    
    % Plot 1: Measured per-chirp power pattern (this IS the Swerling II fluctuation)
    subplot(2,2,1);
    plot(1:N_sweeps_total, chirp_power_dB_norm, 'b-', 'LineWidth', 0.8);
    hold on;
    yline(0, 'r--', 'LineWidth', 1.5, 'Label', 'Mean');
    ...
    if ~isempty(low_snr_idx)
        scatter(low_snr_idx, chirp_power_dB_norm(low_snr_idx), 20, 'r', 'filled');
    end
    ...
    % Plot 3: Histogram of measured power (should match chi-squared(2) in dB domain)
    subplot(2,2,3);
    histogram(chirp_power_dB_norm, 30, 'Normalization', 'pdf', 'FaceColor', [0.3 0.6 0.9], 'EdgeColor', 'w');
    hold on;
    % Note: Chi-squared(2) in linear domain becomes exponential, which in dB has different PDF
    ...
    subplot(2,2,4);
    plot(1:N_sweeps_total, afs_data.local_snr_dB, 'b-', 'LineWidth', 0.8);
    hold on;
    yline(afs_data.threshold_dB, 'r--', 'LineWidth', 1.5, 'Label', 'AFS Threshold');
    ...
    sgtitle(sprintf('Swerling II Target Model - 5.8 GHz (M=%d chirps, Mean RCS=%.1f dBsm)', N_sweeps_total, SWERLING_MEAN_RCS_DBSM));
end
...
if SAVE_BEAT_FILES
    output_filename = 'beat_5_8GHz.mat';
    fs_out = fs;
    % Save the detector’s wide-IF vector (single-sweep processed segment)
    t_out  = t_fast_k;      % match length with beat_signal
